(function ($) {
    window.bookmePro = function (Options) {
        var $container = $('#bookme-pro-form-' + Options.form_id),
            timeZone = typeof Intl === 'object' 'finished') {
            stepComplete();
                        ...
                        function getAvailability() {
                            ...
                            $('.bookme-pro-js-chain-item:not(.bookme-pro-js-draft)', $container).each(function () {
                                ...
                                chain[$chain_item.data('chain_key')] = {
                                    ...
                                    quantity: $('.bookme-pro-js-select-quantity', $chain_item).val() "bookme-pro-calendar-date" />');
                                    $span.html($text);
                                    $(this).html($span);
                                });
                                if (!calendar_loaded) {
                    ...
                    if (response.has_slots) {
                        ...
                        $time_next_button.on('click', function (e) {
                            ...
                            if ($screens.eq(screen_index + 1).length) {
                                $columnizer.animate(
                                    {left: (Options.is_rtl "' + $first_day.attr('value') + '"]', $container).length) {
                                                    $html = $html.not(':first');
                                                }
                        ...
                        $time_prev_button.on('click', function () {
                            ...
                            $columnizer.animate(
                                {left: (Options.is_rtl "tse-content" />');
                        ...
                        while (has_more_slots "bookme-pro-time-screen"/>');
                        ...
                        $columnizer_wrap.height(form_hidden
                            "' + error.failed_key + '"]', $container).addClass('bookme-pro-label-error');
                            } else {
                ...
                success: function (response) {
                    if (response.success) {
                            ...
                            $phone_field.intlTelInput({
                                ...
                                geoIpLookup: function (callback) {
                                    $.get('https://ipinfo.io', function () {
                                        ...
                                        var countryCode = (resp && resp.country) "log"]').val(),
                                    pwd: $login_modal.find('[name="pwd"]').val(),
                                    rememberme: $login_modal.find('[name="rememberme"]').prop('checked') 'json',
                                xhrFields: {withCredentials: true},
                                crossDomain: 'withCredentials' in new XMLHttpRequest(),
                                success: function (response) {
                                    if (response.success) {
                        ...
                        $next_btn.on('click', function (e, force_update_customer) {
                            ...
                            try {
                                phone_number = Options.intlTelInput.enabled 'getNumber') : $phone_field.val();
                                if (phone_number == '') {
                                    ...
                                    if (response.success) {
                                        if (Options.woocommerce.enabled) {
                                            ...
                                            if ($('.bookme-pro-payment[value=local]', $container).is(':checked') || $('.bookme-pro-js-coupon-free', $container).is(':checked') || $('.bookme-pro-payment:checked', $container).val() == undefined) {
                                                ...
                                                $form = $container.find(stripe "card_number"]').val(),
