...
function dd(element, settings) {
	...
	var _construct = function() {		
		...
		_isList = (getElement(_element).size>1 || getElement(_element).multiple==true) "_isList "+_isList);
	...
	var _parseOption = function(opt) {
		...
		if (opt !== undefined) {
			...
			if (attrTitle!="") {
				...
				imagePath = (isJson && _settings.jsonTitle) 'height: 0px;overflow: hidden;position: absolute;',className: _styles.ddOutOfVision};	
	...
	var _createChild = function (opt) {
		...
		var css = (opt.disabled) 'checkbox', name:_element+_settings.checkboxNameSuffix+'[]', value:opt.value||""}); //this can be used for future
			li.appendChild(chkbox);
			if(_settings.enableCheckbox===true) {
				chkbox.checked = (opt.selected) "div", {className: 'clear'});
	...
	var _applyChildEvents = function () {
		...
		$("#" + childid + " li." + _styles.enabled).on("mouseup", function (e) {
			...
			_forcedTrigger = (_oldSelected.length != selected || selected == 0) "#" + childid + " li." + _styles_i.li).on("click", function (e) {
				if (_isDisabled === true) return false;
	...
	var _fixedForDisabled = function () {
		var id = _getPostID("postID");
		var opc = (_isDisabled == true) "#" + id).addClass(_styles.disabledAll);
		} else {
	...
	var _scrollToIfNeeded = function (opt) {
		var childid = _getPostID("postChildID");
		//if scroll is needed
		opt = (opt !== undefined) "#" + childid + " li." + _styles.selected);
		if (opt.length > 0) {
	...
	var _next = function () {
		...
		selected = (selected.length==0) "#" + childid + " li:visible." + _styles_i.li).index(selected);
	...
	var _mergeAllProp = function () {
		...
		_this.selectedText = (_orginial.selectedIndex >= 0) "";
	...
	this.set = function (prop, val) {
		...
		switch (prop) {
			case "size":
				...
				_isList = (getElement(_element).size > 1 || getElement(_element).multiple == true) "multiple":
				getElement(_element)[prop] = val;
				_isList = (getElement(_element).size > 1 || getElement(_element).multiple == true) "disabled":
				getElement(_element)[prop] = val;
				_isDisabled = val;
				_fixedForDisabled();
				break;
	...
	this.visible = function (val) {
		...
		if (val === true) {
			...
			return ($("#" + id).css("display")=="none") "undefined" || r == 0) {
			return false;
		};
